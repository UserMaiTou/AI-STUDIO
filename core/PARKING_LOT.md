...
Status (2026-06-16, updated): canonical templates now authored for `START_HERE.md` and the eight `AI_MISSION/PROJECT_*` records (department-neutral, in `department_templates/AI_MISSION/`), joining `core/AI_ROLE_ROUTING.md`. These surfaces now have a canonical source (`canonical -> export`) and no longer depend on field copies. OPEN — the department `SESSION_PROTOCOL` is NOT canonicalized: a concurrent read-only investigation found the deployed field `SESSION_PROTOCOL` is not a sanitized subset of canonical `core/SESSION_PROTOCOL.md` (it carries non-derivable department-authored content: project-local `AI_STUDIO/`-precedence Source-of-Truth, Project Completion Return, department Mission Pack Rules, added bootstrap/validation). A sanitize-from-canonical would be lossy — a real fork ESCALATED to Human (A: author a parameterized department SESSION_PROTOCOL template = the forked source the no-fork decision avoided; B: leave it `department-authored / no canonical source yet`). The export-protocol Source Map reclassification of the now-canonical surfaces is a Core-protocol edit gated by Finding 3 and is deferred to a Human-approved step, bundled with the SESSION_PROTOCOL decision.
...
Gate:
Finding 2 is RESOLVED by Human Choice A (role/tool-routing alignment). Finding 1's `START_HERE` + eight `PROJECT_*` surfaces are now canonicalized (Human-authorized concurrent pass); the department `SESSION_PROTOCOL` fork is ESCALATED (see Finding 1 status). Still requiring fresh Human Review: the `core/DEPARTMENT_PACK_EXPORT_PROTOCOL.md` Source Map reclassification, the SESSION_PROTOCOL A/B decision, field-pack reconciliation, and any generator / manifest / export automation. The README role / Bootstrap-Status items remain separate Human-approved tasks.
